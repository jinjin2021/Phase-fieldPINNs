...
from CHfourth_equation import CHfourthEquation2D
...
def read_pf_data(time, dLen):
    pf_filename = to_absolute_path(f"Training_data/pf_{int(time):05d}.npz")
    ...
    #outvar["eta"] = np.expand_dims(Image.flatten(), axis=-1)  
...
def run(cfg: ModulusConfig) -> None:
    ...
    # define PDEs
    ch = CHfourthEquation2D(eta="eta", c="c", cAlpha_eq=0.0, cBeta_eq=1.0, M=1.0, dim=2, time=True)
    ...
    batch_size = 1024
    for i, ms in enumerate(np.linspace(10000, 30000, 5)):
        timestep_invar, timestep_outvar = read_pf_data(ms, dLen)
        lambda_weighting = {}
        lambda_weighting["c"] = np.full_like(timestep_invar["x"], 10.0 / batch_size)
    ...
    interior = PointwiseInteriorConstraint(
        nodes=nodes,
        geometry=rec,
        outvar={"CHfourth_equation":0},
        batch_size=4096,
        bounds={x: (0, dLen), y: (0, dLen)},
        lambda_weighting={"CHfourth_equation": 0.0001},
        parameterization=time_range,
    )
    domain.add_constraint(interior, "Interior")
    
    interiorAC = PointwiseInteriorConstraint(
        nodes=nodes,
        geometry=rec,
        outvar={"AllenCahn_equation": 0},
        batch_size=4096,
        bounds={x: (0, dLen), y: (0, dLen)},
        lambda_weighting={"AllenCahn_equation": 0.0001},
        parameterization=time_range,
    )
    domain.add_constraint(interiorAC, "InteriorAC")
...
# c0=0.05, cBeta_eq=1.0
    ...
    ic = PointwiseInteriorConstraint(
        ...
        batch_size=1024,
        bounds=box_bounds,
        #lambda_weighting={"c": 0.01 * time_length},
    ...
    # add validators
    for i, ms in enumerate(np.linspace(35000, 95000, 13)):
